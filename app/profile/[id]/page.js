...
function FeedPost({ 
  ...
  const getProfileImageWithFallback = (userData, size = 'w-12 h-12') => {
    return (
      <div className="relative flex-shrink-0">
        ...
        <div
          className={`${size} bg-gradient-to-br from-blue-400 to-purple-500 rounded-full flex items-center justify-center text-white font-semibold text-sm shadow-sm ${userData?.profile_image ? 'hidden' : 'flex'}`}
        >
  ...
  const { like_count = 0, comment_count = 0, is_liked = false } = feed;
    ...
    <article className="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-100 group">
      ...
      {(like_count > 0 || comment_count > 0) && (
        <div className="px-6 py-2 flex items-center justify-between text-sm text-gray-500 border-t border-gray-50">
          {like_count > 0 && <span>{like_count} คนถูกใจ</span>}
          {comment_count > 0 && <button className="hover:text-blue-600">{comment_count} ความคิดเห็น</button>}
        </div>
      )}
      <div className="px-6 py-3 border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center justify-around">
          ...
          <button className="flex-1 flex items-center justify-center space-x-2 py-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
